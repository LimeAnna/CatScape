...
class Score:
    ...
    def save(self, score: int):
        data = {
            'score': score,
            'date': datetime.now().strftime('%d/%m %H:%M')  # Dia e Mês / Hora e Minutos do Score
        }
    ...
    def show(self):
        ...
        while True:
            ...
            self.score_text(20, "Press ESC to return", COLOR_WHITE, (WIN_WIDTH / 2, 30)) # voltar ao menu principal

            self.score_text(50, "High Scores", COLOR_PINK, (WIN_WIDTH / 2, 100)) # título da tela de scores
